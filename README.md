# Rust Transaction Decoder

Introductory Rust Project which takes in a raw Bitcoin transaction and returns a JSON Object consisting of the segregated Transaction. 

## How to Run

``` shell
$ cargo run -- 
```

Project taken from [here](https://github.com/bitcoin-dev-project/bitcoiner-intro-to-rust).

Note: Currently this transaction decoder only supports non-segwit transactions.
